package supergao.www.recyclerviewanddesign;
...
import supergao.www.recyclerviewanddesign.adapter.ListAdapter;
import supergao.www.recyclerviewanddesign.bean.DATAS;
...
public class MainActivity extends AppCompatActivity implements ListAdapter.OnItemClickListener,SwipeRefreshLayout.OnRefreshListener{
    ...
    private void loadListData(boolean reverse, boolean vertical) {
        ...
        for (int i = 0; i < lenght; i++) {
            DataInfo data = new DataInfo(DATAS.ICONS[i], "list描述" + i);
            dataInfos.add(data);
        }
    ...
    private void loadGridData(boolean reverse, boolean vertical) {
        ...
        int lenght = DATAS.ICONS.length;
        for (int i = 0; i < lenght; i++) {
            DataInfo data = new DataInfo(DATAS.ICONS[i], "grid描述" + i);
            dataInfos.add(data);
        }
    ...
    private void loadStaggered(boolean reverse, boolean vertical) {
        ...
        for (int i = 0; i < lenght; i++) {
            DataInfo data = new DataInfo(DATAS.PICS[i], "瀑布流描述" + i);
            dataInfos.add(data);
        }
    ...
    public void onClick(View itemView, int position) {
        Toast.makeText(this, "已选中第" + (position + 1) + "条", Toast.LENGTH_SHORT).show();
    }
